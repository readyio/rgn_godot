...
onready var okButton: Button = $"VBoxContainer/ButtonOkContainer/ButtonOk"

func _ready():
	okButton.connect("pressed", self, "_onOkButtonPressed")
...
func show_panel(title: String, message: String):
	...
	messageLabel.bbcode_text = "[center]" + message
	self.visible = true
